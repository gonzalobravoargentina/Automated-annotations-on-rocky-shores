...
#confusion matrix new algorithm (EfficientNet extractor) FUNCTIONAL GROUPS
library(here)
CM_NEW_FG <- read.csv(here("/confusion_matrix/New model","confusion_matrix_func_0.csv"),header=FALSE)

##get the Label. It is tricky as the first label has two parenthesis. The valid label is the last one in parenthesis
tblLabels = str_split( CM_NEW_FG [,1], "\\(")
tblLabels = gsub("\\)", "", unlist(lapply(tblLabels, tail,1)))

tt_new_FG = as.table(as.matrix(CM_NEW_FG [,-1]))
rownames(tt_new_FG) = tblLabels
colnames(tt_new_FG) = tblLabels

confusionMatrix(tt_new_FG)
